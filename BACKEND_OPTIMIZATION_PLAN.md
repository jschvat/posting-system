...
if (commentIds.length > 0 && userId) {
  const values = commentIds.map((id, i) =>
    `($${i * 2 + 1}, $${i * 2 + 2}, NOW())`
  ).join(', ');
...
if (mentions.length > 0) {
  await Notification.createBatch(mentions.map(mention => ({
    user_id: mention.id,
    type: 'mention',
...
const responseTimeMiddleware = (req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (duration > 500) { // Log slow responses (>500ms)
...
**High Priority:**
6. `/backend/src/routes/comments.js` (lines 715-754)
7. `/backend/src/routes/groups.js` (lines 668-676)
8. `/backend/src/models/Notification.js` (add createBatch)
9. `/backend/src/routes/posts.js` (GET /:id)
10. `/backend/src/routes/users.js` (GET /:id)
11. `/backend/src/routes/shares.js` (GET /popular)
12. `/backend/src/routes/reactions.js` (GET /user/:userId)

**Medium Priority:**
13. Create migration: `/backend/database/migrations/021_add_performance_indexes.sql`
14. Set up Redis caching infrastructure
15. Add batch endpoints for reactions, shares, comments
16. Create consolidated endpoints (/full, /profile)
17. Add rate limiting middleware
18. Optimize response payload sizes
